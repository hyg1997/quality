...
export default function SettingsPage() {
  ...
  const filteredSettings = settingsCards.filter((setting) => {
    ...
    if (setting.permission && !hasPermission(session, setting.permission))
      return false;
    return true;
  });
  return (
    <div className="space-y-6 px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredSettings.map((setting) => (
          <Link key={setting.href} href={setting.href} className="group block">
            <div className="bg-white shadow-sm rounded-xl border border-gray-100 p-6 hover:shadow-md transition-all duration-200 hover:scale-105">
              <div className="flex items-center space-x-4">
                <div
                  className={`w-12 h-12 ${setting.color} rounded-lg flex items-center justify-center text-white`}
                >
      ...
      <div className="bg-white shadow-sm rounded-xl border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-900">
              {session.user.username || "No configurado"}
            </p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <p className="mt-1 text-sm text-yellow-700">
                Te recomendamos habilitar la autenticación de dos factores para
                mayor seguridad de tu cuenta.
              </p>
            </div>
          </div>
        </div>
